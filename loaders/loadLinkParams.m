...
%% Loads the link parameters for the Dynamic Camera Cluster

% gets the initial guess vector
params_struct = loadDSCParams(file_path);

% Gets a count of the number of DH links
num_DH_links = 0;
...
% Gets transformation from dynamic camera to end effector or J3,
% depending on what parameterization is used
index_offset = 0;
link_struct = [];
link_struct(1).index_map = [-1 -1 -1 -1 -1 -1];
link_struct(1).default_values = zeros(1,6);

% Sets up which parameters to optimize based on flags
...
% Sets up which parameters to optimize based on flags for DH links
...
% Sets up which parameters to optimize based on flags for static parameters
static_cam_count = 0;
