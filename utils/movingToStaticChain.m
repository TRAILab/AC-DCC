...
if strcmp(dcc_obj.link_struct(1).type,'mdh')
    link_opt = dcc_obj.link_struct(1);
    ...
    T_EM = generateMDHMatrix(theta_corrected, link_d, link_a, link_alpha, link_beta, link_y);
    transform_chain{1} = T_EM;
...
if isKey(parameter_container.parameter_key_map, static_cam_key)
    ...
    T_SB = generateFourDOFMatrix(rx_val, ry_val, tx_val, ty_val);
    transform_chain{end+1} = T_SB;
end

% Add the 6 DOF transforms. Transformation from static camera to moving camera
T_SM = T_SB * T_QN_E * T_EM;
end
